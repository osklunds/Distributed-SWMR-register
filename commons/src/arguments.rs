...
pub fn hosts_file_from_matches(matches: &ArgMatches<'static>) -> String {
    matches.value_of("hosts-file").expect("hosts-file not found in matches.").to_string()
}
...
pub fn node_infos_from_string(string: String) -> HashSet<NodeInfo> {
    ...
    for line in string.lines() {
        ...
        let socket_addr = components[1].to_socket_addrs().expect("Could not transform to socket addrs.").next().expect("No socket addrs provided.");
